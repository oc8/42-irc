...
void Server::privmsg_cmd(User &user, std::vector<string> cmds)
{
	...
	for (size_t i = 0; i < targets.size(); i++)
	{
		if (targets[i][0] == '#' || targets[i][0] == '&')
		{
			...
			{
				(*chan).chan_msg_prv(user, ":" + user.get_nickname() + "!~" + user.get_username() + "@127.0.0.1 PRIVMSG " + chan->getName() + " " + message);
			}
		}
		else
		{
			...
			{
				send_msg(*usr, ":" + user.get_nickname() + "!~" + user.get_username() + "@127.0.0.1 PRIVMSG " + (*usr).get_nickname() + " " + message);
			}
		}
	}
}
...
void Server::notice_cmd(User &user, std::vector<string> cmds)
{
	...
	for (size_t i = 0; i < targets.size(); i++)
	{
		if (targets[i][0] == '#' || targets[i][0] == '&')
		{
			chan_it chan = chan_exist(targets[i]);
			if (chan != channels.end())
				(*chan).chan_msg_prv(user, ":" + user.get_nickname() + "!~" + user.get_username() + "@127.0.0.1 NOTICE " + chan->getName() + " " + message);
		}
		else
		{
			usr_ptr usr = user_exist(targets[i]);
			if (usr != NULL)
				send_msg(*usr, ":" + user.get_nickname() + "!~" + user.get_username() + "@127.0.0.1 NOTICE " + (*usr).get_nickname() + " " + message);
		}
	}
}
